...
`;
    ...
    // Save raw SRT (for subtitle burn + timing)
    writeFileSync(join(contentDir, "subtitle.srt"), srt);
    console.log(`     ✅ Saved subtitle.srt`);

    // Extract khutbah start time from first SRT timestamp
    const startMatch = srt.match(/(\d{2}):(\d{2}):(\d{2}),(\d{3})\s*-->/);
    const khutbahStartSeconds = startMatch
      ? parseInt(startMatch[1]) * 3600 + parseInt(startMatch[2]) * 60 + parseInt(startMatch[3])
      : 25; // default fallback
    console.log(`     ⏱️ Khutbah starts at: ${khutbahStartSeconds}s`);
    ...
    const metadata = {
      ...
      khutbah_start_seconds: khutbahStartSeconds,
      status: "fetched",
